(function(){
  ...
  function fixedVereadorColor(l){var v=String((l&&l.vereadorNome)||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim().toLowerCase();if(v==='marcelo junio')return '#f97316';if(v==='meiry farias'||v.indexOf('meiry farias ')===0)return '#16a34a';return null;}
  function leadershipColor(l){var fixed=fixedVereadorColor(l);if(fixed)return fixed;var name=String((l&&l.nome)||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim().toLowerCase();var key=String((l&&l.vereadorId)||'')+'|'+name;var h=0;for(var i=0;i<key.length;i++)h=((h<<5)-h)+key.charCodeAt(i)|0;return teamPalette[Math.abs(h)%teamPalette.length];}
